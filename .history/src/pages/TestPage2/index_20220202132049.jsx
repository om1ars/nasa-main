	...
	$.Gallery.prototype 	= {
		...
		_setItems			: function() {
			...
			if( this.itemsCount > 3 ) {
				...
				// next item
				this.$nextItm		= ( this.$rightItm.index() === this.itemsCount - 1 ) ? this.$items.eq( 0 ) : this.$rightItm.nextElementSibling;
				this.$nextItm.css( this._getCoordinates('outright') );
				
				// previous item
				this.$prevItm		= ( this.$leftItm.index() === 0 ) ? this.$items.eq( this.itemsCount - 1 ) : this.$leftItm.previousElementSibling;
		...
		_loadEvents			: function() {
			...
			this.$wrapper.addEventListener( 'webkitTransitionEnd.gallery transitionend.gallery OTransitionEnd.gallery', function( event ) {
		...
		_navigate			: function( dir ) {
			...
			switch( dir ) {
				...
				case 'next' :
					...
					this.$currentItm.classList.add('dg-transition').css( this._getCoordinates('left') );
					
					// right item moves to the center
					this.$rightItm.classList.add('dg-transition').css( this._getCoordinates('center') );	
					
					// next item moves to the right
					if( this.$nextItm ) {
						
						// left item moves out
						this.$leftItm.classList.add('dg-transition').css( this._getCoordinates('outleft') );
						
						this.$nextItm.classList.add('dg-transition').css( this._getCoordinates('right') );
						
					}
					else {
					
						// left item moves right
						this.$leftItm.classList.add('dg-transition').css( this._getCoordinates('right') );
				...
				case 'prev' :
					...
					this.$currentItm.classList.add('dg-transition').css( this._getCoordinates('right') );
					
					// left item moves to the center
					this.$leftItm.classList.add('dg-transition').css( this._getCoordinates('center') );
					
					// prev item moves to the left
					if( this.$prevItm ) {
						
						// right item moves out
						this.$rightItm.classList.add('dg-transition').css( this._getCoordinates('outright') );
					
						this.$prevItm.classList.add('dg-transition').css( this._getCoordinates('left') );
						
					}
					else {
					
						// right item moves left
						this.$rightItm.classList.add('dg-transition').css( this._getCoordinates('left') );
			...
			if( !this.supportTrans )
				this.$currentItm.classList.add('dg-center');
